...
Reset history compares local Token consumption in a horizontally scrollable chart, with aligned values and units. The latest period is selected on opening. Click a period or use the arrow keys to change the exact time, recorded quota percentage and source shown in the detail line; moving the pointer or scrolling preserves your selection. Drag to browse, or use the scrollbar and arrow keys. Missing readings show a dash. Scheduled, Manual and Official remain separate; Official uses purple and is inferred from recovery outside scheduled or confirmed in-app manual resets. The public [Codex Reset Today API](https://codex-reset.today/developers) provides an optional global-reset forecast below the account countdown, with a time window, approximate probability and confidence/source in the tooltip. It is not an account guarantee; stale or unavailable forecasts are hidden. Requests send no account or task data. Using a reset credit still requires confirmation.
