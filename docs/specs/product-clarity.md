...
- Week and Today retain their local Token destinations with two header rows: a concise cycle/today title, last Token update time and unit switch, then the period and sum. Use `usage_at` for the Token update time, not the quota API timestamp; show a date only when needed and keep full timestamps in the tooltip. Local-only scope, account quota and daily observation coverage remain in tooltips instead of repeated visible rows. Do not estimate missing usage or change reset-delta arithmetic.
...
History rows show bare percentages such as `91%`; the shared tooltip explains that they represent recorded account usage. Missing records display `—`. The expiry section explicitly identifies reset credits, rather than showing an ambiguous generic expiry heading.
