...
履歴はリセット間の合計使用量を比較します。棒の色は定期が青、公式が紫、手動が青緑で、単位は見出しにまとめます。日付はリセットの境界を示し、ドラッグやスクロールで移動できます。選択して開く詳細はなく、未記録の値はダッシュで表示します。公式は定期と確認済みの手動操作を除いた推定分類です。パネルにはアカウントのリセット時刻と利用可能なリセット権を表示します。リセット権の使用には確認が必要です。
