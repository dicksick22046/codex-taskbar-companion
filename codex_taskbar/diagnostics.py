...
def diagnostic_text(settings,data,window):
    return json.dumps({
        ...
        'presentation':{key:settings.get(key) for key in ('placement','capsule_theme','capsule_transparency','language','rotate_quotas','show_week','show_session','show_daily','show_countdown','show_tasks','show_task_strip','show_task_statistics','hover_panels','notify_input')},
