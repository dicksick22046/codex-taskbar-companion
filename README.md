...
| Status dot and count | Opens the task directly when the count is 1; otherwise opens the status list |
| Task summary row (except its close button) | That task in Codex |

Running, input-needed, unread and failed task summaries appear automatically above the bar. Each nonempty state has one rotating row. Hover the row or focus its close button with the keyboard to dismiss the current group; ordinary refreshes and restarts do not reopen it, while a new task, execution or state entry does. Other parts of the row open the task. Details temporarily replace the summaries and restore them on close. Side chats open their parent task.
...
- Select **USD** in cycle/today usage, or use the reset-history heading selector, to view estimated Standard API-equivalent costs. Rates are model-specific, reviewed on 2026-09-16, and include cached input and long-context pricing. This is not a subscription bill; missing model or per-call details remain a dash. See the [calculation rules](docs/specs/usage-cost.md).
...
Reset history compares totals between resets. Bar colors identify Scheduled (blue), Official (purple) and Manual (teal), with the unit shown once in the heading. Dates mark reset boundaries; drag or scroll to browse, with no extra selection details. Missing readings remain a dash. Official resets are inferred from recovery outside scheduled or confirmed in-app manual resets. The optional [Codex Reset Today forecast](https://codex-reset.today/developers) concerns global resets, not an account guarantee; unavailable forecasts are hidden. Using a reset credit still requires confirmation.
