...
- Preserve valid cached readings on quota-read failure, but show a quiet, non-blinking `Cached` indicator near the metrics. Do not add long explanatory tooltips to panels or navigation entries. Other task-metadata errors do not falsely label successful quota data as failed.
...
- Week and Today retain their local Token destinations with two header rows: a concise cycle/today title, last Token update time and unit switch, then the period and sum. Use `usage_at` for the Token update time, not the quota API timestamp; show a date only when needed. Do not add panel-wide explanatory hover paragraphs; navigation hints remain short. Do not estimate missing usage or change reset-delta arithmetic.
