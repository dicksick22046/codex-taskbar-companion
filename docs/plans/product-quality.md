...
| 1.11 Reset history overview and selected-period details | Implemented, pushed and locally installed; Unreleased | Aligned values, common column baseline, and one fixed time/quota/source detail line. Hover preview, persistent click selection, drag cancellation, visible keyboard selection and interruptible motion. 354 native tests (1 skipped), four offscreen journeys, multilingual/overflow renders and main CI pass. Installed build/preferences verified; public 0.10.1 unchanged. See [review](../qa/pinned-status.md#unreleased-reset-overview-and-selection). |
