...
历史用量保留每行数值、单位及有记录的重置前已用占比；缺失的旧百分比不补造。自然、手动、官方分别显示，官方用紫色，是排除自然到期和本工具已确认手动重置后的推定分类。下次重置下方可显示 [Codex Reset Today](https://codex-reset.today/developers) 的第三方全局预测，包括时间范围和大致概率，提示中注明置信度和来源；它不是个人账号的保证，过期或不可用时隐藏，不发送账号或任务数据。使用重置机会仍需二次确认。
