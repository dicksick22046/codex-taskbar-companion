...
| 1.11 Reset history comparison chart with recorded quota percentages | Implemented, pushed and locally installed; Unreleased | Periods on a fixed-height horizontal timeline, shared Token column scale, exact units, quota/source and timestamp tooltips. Native horizontal scrollbar, touchpad/wheel/keyboard support; vertical access remains available on short screens. 348 native tests (1 skipped), four offscreen journeys, multilingual/overflow renders and main CI pass. Installed build and preferences verified; public 0.10.1 unchanged. |
