...
The user subsequently selected a horizontal comparison timeline, delivered locally from `0a09dce`. Periods now use shared-scale vertical columns in a fixed-height chart, with a native horizontal scrollbar when needed. Verification covers pixel-wheel movement, mouse-wheel routing, native thumb dragging via direct widget events, keyboard boundaries, refresh-position retention, missing/zero amounts and timestamp tooltips. All 348 native tests pass with one skip; four separate offscreen journeys and Windows/portable CI pass. Multilingual start/end renders were inspected, and the installed executable matches the build with normalized preferences and native attached windows preserved. The public 0.10.1 assets remain unchanged.
