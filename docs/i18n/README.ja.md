...
履歴は週の使用量グラフと同じ棒、数値ラベル、日付軸を使い、期間を左右にスクロールして比較できます。開くと最新の期間を選択します。クリックまたは左右キーで時刻、利用枠使用率、リセット種別を切り替え、ホバーやスクロールでは選択を変えません。ドラッグ、スクロールバー、左右キーで移動でき、未記録の値はダッシュで表示します。定期、手動、公式を区別し、推定された公式リセットは紫色です。[Codex Reset Today](https://codex-reset.today/developers) の全体リセット予測を時間範囲と概算確率で表示し、信頼度と出典はツールチップに記載します。個人アカウントの保証ではなく、古いデータや取得できない予測は隠れます。アカウントやタスクの情報は送信しません。リセット権の使用には確認が必要です。
