...
状態一覧の右上にあるピンで、バーの上に1行を固定できます。複数の状態は1つのパネルにまとめ、各行には色付きの点、プロジェクト名、タスク名、解除ボタンを表示します。各行は独立して切り替わり、ホバー中は強調表示して切り替えを止め、長い名前を読み進めます。空の行は設定を保持したまま隠れます。以前有効だった実行中バーは固定行へ移行します。Main/Side の詳細は一覧とツールチップで確認できます。
