...
| 1.11 Reset history Token bar chart with independent quota percentages | Implemented, pushed and locally installed; Unreleased | One 28-DIP row per period: inline shared-scale Token bar, amount/unit, recorded quota and source. Four-language renders, 79 focused follow-up checks, main CI and matching installed build with preserved preferences. Constrained-height scrolling retains its fixed header/button. Public 0.10.1 remains unchanged. |
