...
历史用量用横条对比各周期的本机Token消耗，每行保留数值和单位；额度百分比独立显示，缺失记录为横杠。横条长度不代表账户额度。自然、手动、官方分别显示，官方用紫色，是排除自然到期和本工具已确认手动重置后的推定分类。下次重置下方可显示 [Codex Reset Today](https://codex-reset.today/developers) 的第三方全局预测，包括时间范围和大致概率，提示中注明置信度和来源；它不是个人账号的保证，过期或不可用时隐藏，不发送账号或任务数据。使用重置机会仍需二次确认。
