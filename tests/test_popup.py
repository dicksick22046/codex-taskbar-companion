...
class Owner(QWidget):
    ...
    language='en'
    @property
    def navigation_width(self):return self.width()
    @property
...
class PopupInitialFrameTests(unittest.TestCase):
    ...
    def test_task_hit_area_covers_visible_row_and_keeps_text_clear_of_pin(self):
        ...
        self.assertFalse(strip.task_at_point(QPointF(strip.close_button.geometry().center())))
        ...
        with patch.object(bar,'open_task') as opened:
            blank=QPointF(strip.close_button.x()-3,15)
            ...
            pin=QPointF(strip.close_button.geometry().center())
        ...
        self.assertLessEqual(strip.task_rect.right(),strip.close_button.x()-8)
        self.assertGreater(strip.task_rect.left(),30)
        self.assertTrue(strip.task_at_point(QPointF(strip.close_button.x()-3,15)))
