...
| 1.11 Reset history using the weekly chart renderer | Implemented, pushed and locally installed; Unreleased | Shared columns, value labels and date axis; 8 weekly reference images are pixel-identical. Defaults to latest, click/keyboard selection, date-only highlight, no hidden scrollbar spacer. 356 native tests (1 skipped), four offscreen journeys, multilingual/overflow renders and main CI pass. Installed build/preferences verified; public 0.10.1 unchanged. See [review](../qa/pinned-status.md#unreleased-reset-overview-and-selection). |
