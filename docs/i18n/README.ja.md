...
| 状態の点と件数 | 1件ならタスクを直接開き、複数なら状態一覧を開く |
| タスク名 | Codex で該当タスクを開く |

実行中・回答待ち・未読・失敗の要約をバーの上に自動表示します。各状態は1行で切り替わり、空なら隠れます。ホバーまたはキーボードフォーカスで閉じるボタンが現れ、現在のグループだけを非表示にします。通常の更新や再起動では再表示せず、新しいタスク、実行、状態への移行で再表示します。行のその他の部分からタスクを開けます。詳細を閉じると現在の要約に戻ります。

詳細パネルは同じ接続領域に開き、固定行を一時的に置き換えます。閉じると固定行が戻り、ピンの設定は変わりません。
...
週と5hはアカウントの残量、Todayは観測開始後の使用量です。取得失敗時は有効な記録を保持し、接続状態は設定で確認できます。期限切れの残量やリセット時刻は不明として扱います。ローカルのトークン合計からアカウント利用枠を正確に逆算することはできません。
...
- 期間・日別使用量の **USD**、またはリセット履歴の見出しから、標準 API 料金に換算した推定費用を表示できます。2026-09-16 に確認したモデル別料金を使用し、キャッシュと長い入力を区別します。サブスクリプションの請求額ではありません。モデルや呼び出しごとの明細が不明な場合はダッシュを表示します。

履歴はリセット間の合計使用量を比較します。棒の色は定期が青、公式が紫、手動が青緑で、単位は見出しにまとめます。日付はリセットの境界を示し、ドラッグやスクロールで移動できます。選択して開く詳細はなく、未記録の値はダッシュで表示します。公式は定期と確認済みの手動操作を除いた推定分類です。任意の [Codex Reset Today予測](https://codex-reset.today/developers)は全体の予測で、個人アカウントの保証ではありません。リセット権の使用には確認が必要です。
