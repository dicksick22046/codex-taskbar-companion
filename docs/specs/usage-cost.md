...
新一轮任务开始后的首条有效 token_count，若当前累计值与 last_token_usage 在总量、输入、缓存读取、缓存写入和输出各字段完全相同，且总量等于输入加输出、缓存拆分不超过输入，则它证明本轮计数从一次完整调用重新起算；忽略上一轮的计数作为本轮基线，不论新数值是否大于旧数值。Token 与美元共用这一边界，避免美元金额和 Token 柱子口径不一致。没有新一轮证据、明细不匹配或计数自相矛盾时仍按原累计差分并保留美元未知；不推断多次调用的模型或长上下文分段价格。
