...
| 1.11 Reset history overview and selected-period details | Implemented, pushed and locally installed; Unreleased | Compact aligned values and a fixed detail line. Defaults to latest history; only clicks/keyboard change selection, hovering and scrolling preserve it. Drag cancellation and interruptible motion remain. 355 native tests (1 skipped), four offscreen journeys, multilingual/overflow renders and main CI pass. Installed build/preferences verified; public 0.10.1 unchanged. See [review](../qa/pinned-status.md#unreleased-reset-overview-and-selection). |
