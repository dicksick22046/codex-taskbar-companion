...
打开任一状态列表，点击右上角图钉，即可在底栏上方固定一行。多个状态合成一个小面板，每行只显示颜色圆点、任务标题和取消固定按钮；各自轮换，悬停有高亮、暂停切换并展开长标题。没有任务时隐藏对应行，保留固定偏好。旧版已开启的进行中悬浮条会迁移为固定进行中。主侧任务信息保留在完整列表和提示中，点击侧聊返回父任务。
