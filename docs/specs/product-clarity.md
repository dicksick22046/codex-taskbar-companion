...
- Reset history displays the existing three categories: Scheduled, Manual and Official. Official remains an inferred classification for recovery outside a scheduled reset or confirmed in-app manual reset; do not add a fourth visible category. A single Token column heading replaces the repeated row caption; numbers stay right-aligned with per-row 100M units. The reset panel starts with Next reset; the panel has no long hover explanation.
...
History rows show bare percentages such as `91%`; these represent recorded account usage. Missing records display `—`. The expiry section explicitly identifies reset credits, rather than showing an ambiguous generic expiry heading.
