...
- Preserve valid cached readings on quota-read failure without a visible `Cached` label or reserved label space in navigation or the 5h panel. Settings retain the existing last-record status. Do not add replacement indicators or long explanatory tooltips. Expiry handling and refresh cadence are unchanged; other task-metadata errors do not falsely label successful quota data as failed.
...
- Week and Today retain their local Token destinations with two header rows: a concise cycle/today title, last Token update time and unit switch, then the period and sum. Use `usage_at` for the Token update time, not the quota API timestamp; show a date only when needed. Do not add explanatory hover text to panels or navigation. Do not estimate missing usage or change reset-delta arithmetic.
...
- Reset history presentation follows [Reset-period comparison](interaction.md#reset-period-comparison): slimmer type-colored bars, one heading unit/legend, and scrolling without click selection or a percentage detail row. Keep original account before/after snapshots; Official remains an inferred classification outside scheduled or confirmed manual resets. Do not invent values for missing history.

Cycle/today sums remain beside their period labels. The expiry section identifies reset credits explicitly.
...
- Add a checkable `History statistics` control with accessible name, description and keyboard focus, without visible tooltip text. When expanded, reveal Run time, Tokens, Turns and the existing unit control in the same table. Preserve every calculation and cache. A `show_task_statistics` preference saves this explicit choice; default false, including upgrades because there was no prior visibility preference.
