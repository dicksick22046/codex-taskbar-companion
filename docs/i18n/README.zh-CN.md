...
历史用量直接复用周用量图的柱体、数值标注和日期轴，更多周期可横向浏览。打开时默认选中最近一次历史，点击或键盘选择后切换精确时间、额度百分比和来源；鼠标经过及滚动不改变选择；拖动图表、滚动条或左右键均可浏览。缺失记录显示横杠。自然、手动、官方分别显示，官方用紫色，是排除自然到期和本工具已确认手动重置后的推定分类。下次重置下方可显示 [Codex Reset Today](https://codex-reset.today/developers) 的第三方全局预测，包括时间范围和大致概率，提示中注明置信度和来源；它不是个人账号的保证，过期或不可用时隐藏，不发送账号或任务数据。使用重置机会仍需二次确认。
