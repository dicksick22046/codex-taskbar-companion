...
使用量の履歴ではローカルのToken消費量を横棒で比較し、各行に数値と単位を表示します。利用枠使用率は別の列に表示し、記録がない場合はダッシュにします。棒の長さは利用枠の割合ではありません。定期、手動、公式を区別し、推定された公式リセットは紫色です。[Codex Reset Today](https://codex-reset.today/developers) の全体リセット予測を時間範囲と概算確率で表示し、信頼度と出典はツールチップに記載します。個人アカウントの保証ではなく、古いデータや取得できない予測は隠れます。アカウントやタスクの情報は送信しません。リセット権の使用には確認が必要です。
